...
BREAKING_PATTERNS = [
    ...
    BreakingChangePattern(
        ...
        file_types=[".py", ".sql", ".scala"],
        description="[Review] JDK 17 strictly enforces datetime pattern width. 'MM' requires 2-digit months, 'dd' requires 2-digit days, 'yy' requires 2-digit years. On standard clusters: returns NULL. On Serverless (ANSI mode): throws CANNOT_PARSE_TIMESTAMP. Note: M/d/y (single y) misinterprets 2-digit years as 0022.",
    ...
    BreakingChangePattern(
        id="BC-17.3-003",
        name="Spark Connect Literal Handling (Review) - Scala",
        severity="LOW",
        introduced_in="17.3",
        pattern=r"(?<!\.)(?:array|map|struct)\s*\(",
        file_types=[".scala"],
        description="[Review] Spark Connect 17.3: null values preserved, decimal precision changed to (38,18)",
        remediation="Handle nulls explicitly with coalesce(); specify decimal precision if needed"
    ),
